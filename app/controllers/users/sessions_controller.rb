...
class Users::SessionsController < Devise::SessionsController
  ...
  def show
    if current_user
      ...
      @users = User.all.select(:id, :string_id)
    end
    render :json => {:current_user_string_id => current_user_string_id, :my_ask_count => my_ask_count, :my_vote_count => my_vote_count, :my_comment_count => my_comment_count, :in_progress_count => in_progress_count, :alram_count => alram_count, :alrams => @alrams, :users => @users}
  end
