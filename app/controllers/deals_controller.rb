  ...
  def create_by_naver
    ...
    if redirect_link.split("'")[1].include?('http')
      ...
      if !naver_shop_doc.css('.info_inner span').empty?
        ...
        brand2 = naver_shop_doc.css('.info_inner span')[1].text
      elsif !naver_shop_doc.css('.tit span').empty?
    ...
    preview_image = PreviewImage.create(user_id: current_user.id,
                                        image: naver_deal[:image])
